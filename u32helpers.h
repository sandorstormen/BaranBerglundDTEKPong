...
uint8_t spi_send_recv2(uint8_t data);
...
/* Declare display_debug - a function to help debugging.
   ...
   repeated calls to display_image2; display_image2 overwrites
...
void display_image2(int x, const uint8_t *data) {
	...
	for(i = 0; i < 4; i++) {
		...
		spi_send_recv2(x & 0xF);
		spi_send_recv2(0x10 | ((x >> 4) & 0xF));
		
		DISPLAY_CHANGE_TO_DATA_MODE;
		
		for(j = 0; j < 32; j++)
			spi_send_recv2(~data[i*32 + j]);
	}
}
